...
onInit()
{
    ...
    level.fpsHudName = "fps";
    openCJ\huds\infiniteHuds::initInfiniteHud(level.fpsHudName);
}

onPlayerConnect()
{
    //                                     name             x        y    alignX         alignY    hAlign        vAlign
    self openCJ\huds\base::initInfiniteHUD(level.fpsHudName, -5,     0,    "right",    "top",    "right",    "top",
...
onFPSChanged(newFPS)
{
    ...
    if (self.sessionState == "playing")
    {
        ...
        {
            specsAndSelf[i].hud[level.fpsHudName] openCJ\huds\infiniteHuds::setInfiniteHudText(newFPSText, specsAndSelf[i], false);
        }
    }
    ...
    {
        self.hud[level.fpsHudName] openCJ\huds\infiniteHuds::setInfiniteHudText(newFPSText, self, false);
    }
}
...
onSpectatorClientChanged(newClient)
{
    ...
    self.hud[level.fpsHudName] openCJ\huds\infiniteHuds::setInfiniteHudText(currentFPSText, self, false);
}
...
_onFPSHudSetting(newVal)
{
    ...
    if(shouldEnable)
    {
        ...
        currentFPSText = "" + self openCJ\fps::getCurrentFPS();
        self.hud[level.fpsHudName] openCJ\huds\infiniteHuds::setInfiniteHudText(currentFPSText, self, false);
        self openCJ\huds\base::enableHUD(level.fpsHudName);
    }
    else
    {
        //self setClientCvar("cg_drawfps", 1);
        self openCJ\huds\base::disableHUD(level.fpsHudName);
    }
}
